...
eve_tools — EVE Online 工业工具箱，纯前端 SPA（无后端）。包含蓝图材料计算器、市场价格查询、旗舰跳跃路线规划、公开合同查询和虫洞系统查询。支持国服 Serenity 和世界服 Tranquility（通过 /gf 和 /of 路由区分）。致谢页面 `/credits` 列出所有数据来源。
...
- **部署**: 任何静态托管（Cloudflare Pages / Vercel / GitHub Pages）
...
所有功能页通过 `/:server(gf|of)/` 动态路由前缀区分服务器。`stores/settings.js` 持久化 server 和 locale 到 localStorage。`i18n.js` 提供双语翻译。
...
node scripts/convert-sde.mjs --fetch-zh-names     # 同时从 Serenity ESI 获取中文地图名
