...
- Side-by-side tier columns: Final Product, every tier and the raw summary render simultaneously, wrapping automatically when more tiers exist than fit one row / 最终产品 / 每级材料 / 原材料汇总并列显示，一行放不下自动换行（不再需要切换标签）
...
- Compact "EFT" swap-arrow button beside the ship search — paste the in-game "Copy to clipboard" output (or any Pyfa EFT) to load a fit, even before picking a hull / 船型搜索旁内置紧凑 "EFT" 按钮，直接粘贴游戏内"复制到剪贴板"输出或 Pyfa EFT 文本即可导入（选船前也能用）
